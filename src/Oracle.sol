...
import { IOracle, IClient } from "./interfaces.sol";
...
contract Oracle is IOracle {
  ...
  event ResultAvailable(uint256 indexed id, string urlToQuery, string attributeToFetch, string agreedValue);

  function createRequest(string memory _urlToQuery, string memory _attributeToFetch) external returns (uint256) {
    uint256 requestId = currentId++;

    requests.push(Request(requestId, msg.sender, _urlToQuery, _attributeToFetch, ""));
    mapping(address => uint256) storage quorum = requestQuorum[requestId];
    ...
    emit NewRequest(requestId, _urlToQuery, _attributeToFetch);

    return requestId;
  }

  // called by (off-chain) oracle account to record its answer
  function updateRequest(uint256 _id, string memory _valueRetrieved) external {
      ...
      for (uint256 i = 0; i < totalOracleCount; i++) {
        ...
        if (keccak256(a) == keccak256(b)) {
          ...
          if (currentQuorum >= minQuorum) {
            ...
            if (currRequest.requester.code.length > 0) {
              // requester is a smart contract
              IClient client = IClient(currRequest.requester);
              client.requestCallback(currRequest.id, currRequest.agreedValue);
            } else {
